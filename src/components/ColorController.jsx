...
export function ColorController({ devices, selectedDeviceIds, onToggleDevice, token, secret, spotifyToken, spotifyClientId, onOpenSettings, onTokenExpired }) {
    ...
    // ... (rest of simple Spotify logic) ...

    // --- REPLACED: Old Sync Logic Eliminated --- 
    // The hook handles loop, video, and color extraction now.
    // We only need to handle the SwitchBot API throttling here.

    // ... (keep Spotify fetchStatus, syncWithSpotify, etc) ...

    // Auto-Sync Polling (Simplified)
    useEffect(() => {
        let interval;
        if (autoSync && power) {
            ...
            // Screen Sync loop is now internal to the hook! No set interval needed here!
        }
        return () => {
    ...
    useEffect(() => {
        if (syncMode === 'screen' && !isSharing) {
            // If user switched to screen mode but not sharing, maybe auto-start? 
            // No, let them click button.
        }
